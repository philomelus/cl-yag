...
(defstruct (row-layout-tests-data (:include tests-data)
  ...
  st311 st312 st321 st322 st331 st332 rl3 t31 t32 t33
...
(defmethod tests-create ((data (eql *row-layout-data*)))
  (let (widgets)
    (with-slots (manager w1 w2 w3 w4
      ...
      ;; Test 1
      (setf st111 (deftests-status :wide 1 3 2 "[HA1] :RIGHT" 1 1))
      (setf st112 (deftests-status :wide 1 3 2 "[VA1] :MIDDLE" 1 2))
      (setf st121 (deftests-status :wide 1 3 2 "[HA2] :CENTER" 2 1))
      (setf st122 (deftests-status :wide 1 3 2 "[VA2] :MIDDLE" 2 2))
      (setf st131 (deftests-status :wide 1 3 2 "[HA3] :LEFT" 3 1))
      (setf st132 (deftests-status :wide 1 3 2 "[VA3] :MIDDLE" 3 2))
      ...
      ;; Test 2
      (setf st211 (deftests-status :wide 2 3 2 "[HA1] :RIGHT" 1 1))
      (setf st212 (deftests-status :wide 2 3 2 "[VA1] :MIDDLE" 1 2))
      (setf st221 (deftests-status :wide 2 3 2 "[HA2] :CENTER" 2 1))
      (setf st222 (deftests-status :wide 2 3 2 "[VA2] :MIDDLE" 2 2))
      (setf st231 (deftests-status :wide 2 3 2 "[HA3] :LEFT" 3 1))
      (setf st232 (deftests-status :wide 2 3 2 "[VA3] :MIDDLE" 3 2))
      ...
      ;; Test 3
      (setf st311 (deftests-status :wide 3 3 2 "[1]" 1 1))
      (setf st312 (deftests-status :wide 3 3 2 "[1]" 1 2))
      (setf st321 (deftests-status :wide 3 3 2 "[2]" 2 1))
      (setf st322 (deftests-status :wide 3 3 2 "[2]" 2 2))
      (setf st331 (deftests-status :wide 3 3 2 "[3]" 3 1))
      (setf st332 (deftests-status :wide 3 3 2 "[3]" 3 2))
      (setf t31 (deftext :title "Title 3/1" :v-align :middle :h-align :center))
      (setf (border t31) (defborder :thickness 2))
      (setf (spacing t31) 5)
      (setf t32 (deftext :title "Title 3/2" :v-align :middle :h-align :center))
      (setf (border t32) (defborder :thickness 2))
      (setf (spacing t32) 5)
