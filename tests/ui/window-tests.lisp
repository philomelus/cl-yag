...
(defmethod tests-create ((data (eql *window-data*)))
  (let (widgets)
    (with-slots (manager w1 w2 w3 w4 w5 w6 w7 w8
      ...
      (setf st15 (deftests-status :wide 2 5 2 "3D: DEFAULT" 4 1))
      (setf w3 (deftests-window :wide 2 :content `(,st11 ,st12 ,st13 ,st14 ,st15)))
      (push w3 widgets)
  ...
  (defmethod tests-command-update ((data (eql window-data)))
    (with-slots (st11 st12 st13 st14 st15 w1 theme2) data
      (unless (eql st11 nil)
        ...
        (setf (title st15) (format nil "3D: ~a" (style theme2)))
        )))
