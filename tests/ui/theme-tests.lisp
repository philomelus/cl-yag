...
(defstruct (theme-tests-data (:include tests-data)
                            (:conc-name theme-tests-))
  w1 w2 w3 w4 w5 w6 w7 w8
  )

(defparameter *theme-data* (make-theme-tests-data))

(defmethod tests-create ((data (eql *theme-data*)))
  (let (widgets)
...
(defmethod tests-destroy ((data (eql *theme-data*)))
  (let ((args `((eql ,data))))
    (cleanup-method tests-get-interior-color args))
  nil)

(defmethod tests-ready ((theme-data (eql *theme-data*)))
  (defmethod tests-get-interior-color ((data (eql theme-data)))
    (with-slots (w1 w2 w3 w4 w5 w6 w7 w8) data
      (values (list w1 w2 w3 w4 w5 w6 w7) nil)))
  
  nil)

(defun theme-tests-main ()
  (tests-main *theme-data*))
